...
3. **Most of the gain comes from repeated attempts, not from pruning.** Roughly 60-70%
   ...
   more than 16 of its 38 extra cubes. A genuine but smaller pruning effect accounts for
   the rest; the best one-pass hybrid keeps 11 of the 38 (Finding 8e).
...
- **What this establishes, and what it does not.** A test asserts the hybrid equals
  ...
  *how*. The hybrid makes several passes, each pruned differently under a different
  ...
  afterwards in Finding 8e:** roughly 60-70% of the gain comes from the several attempts,
...
- **Several differently-pruned attempts account for most of the gain.** Of the 38
  cubes the hybrid solves and beam search does not, the best one-pass hybrid (bound 30)
  solves 11 (29%). The single bound that recovers the most of them is 12, with 16 (42%)
  -- but that pass loses 26 cubes beam search solves, so overall it solves 10 fewer
  than beam search does. Either way, at most 16 of the 38 can come from any one bound:
  **roughly 60-70% of the gain needs passes at several different bounds.**
- **Different bounds help different cubes.** Recovery peaks twice: around bounds 11-12
  (13 and 16 of the 38) and again at the loosest bounds (11 at bound 30), with few in
  between and none at bounds 17-18.
...
- **This explains the equal-time results in Findings 8b-8d.** Roughly 60-70% of the
...
**Figure:** `figures/why_hybrid_wins.png`, generated by
`notebooks/03_why_hybrid_wins.ipynb`. The left panel plots each single pass's change in
...
3. **Why: most of the gain comes from repeated attempts, not from pruning** (Finding
   ...
   one-pass hybrid is cheap but keeps only 11 of the 38 extra cubes, and no single bound
   recovers more than 16.
